...
**最もシンプルで確実な方法**
- このスクリプトを**一度だけ実行**すると、`setup_and_run.sh`が次回起動時から自動実行されるように設定されます
- ラズパイ起動時に自動で`setup_and_run.sh`が呼ばれるようになります
...
より現代的な方法。機能は`install_autostart.sh`と同じです。
```bash
...
/home/tsukimi/tsukimi-speaker-rust/
  ├── setup_and_run.sh
  ├── install_autostart.sh         # ← これを使います（推奨）
  ├── install_autostart_systemd.sh # または、こちら
  ├── src/
  ├── Cargo.toml
  ├── *.mp3  (全BGM・SEファイル)
  └── その他プロジェクトファイル
...
**これ以降は何もしなくてOK！電源を入れるだけで自動で起動します。**
